...
export default function Home() {
  ...
  const [isResponseBodyExpanded, setIsResponseBodyExpanded] = useState(false);
  ...
  const [loadTestResults, setLoadTestResults] = useState<{
    ...
    history: { index: number; timeMs: number; status: number }[];
  } | null>(null);
  const [loadTestConfig, setLoadTestConfig] = useState({ requests: 50, concurrency: 10 });
  ...
  const runLoadTest = async () => {
    ...
    const { requests, concurrency } = loadTestConfig;
    let completed = 0;
    let successCount = 0;
    let failCount = 0;
    const history: { index: number; timeMs: number; status: number }[] = [];
    let currentIndex = 0;

    // Create base request details same as handleHello
    ...
    activeTab?.headers.filter(h => h.isActive && h.key).forEach(h => { requestHeaders[h.key] = h.value; });
    ...
    const requestOptions: RequestInit = { method: selectedMethod, mode: "cors", credentials: "omit", headers: requestHeaders };
    ...
    if (["POST", "PUT", "PATCH", "DELETE"].includes(selectedMethod)) {
      ...
      // Skip form-data/xml for load testing simplicity here
    }

    const runWorker = async () => {
      while (currentIndex < requests) {
        const reqIndex = currentIndex++;
        if (reqIndex >= requests) break;
        ...
        history.push({ index: reqIndex + 1, timeMs, status });
        completed++;

        // Calculate running stats
        const times = history.map(h => h.timeMs).sort((a, b) => a - b);
        ...
        setLoadTestResults({
          ...
          p95Time: times[p95Idx] || 0,
          currentProgress: (completed / requests) * 100,
          history: [...history], // shallow copy for react
        });
      }
    };
          ...
          <div className="">
            <svg width="80" height="80" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <g clipPath="url(#clip0_4418_6503)">
                <path opacity="0.4" d="M16.1599 10C16.1599 10 16.0099 11 12.9099 15C9.9999 18.77 14.2099 21.64 14.7099 21.97C14.7399 21.99 14.7699 21.99 14.8099 21.97C15.4899 21.55 23.0599 16.68 16.1599 10Z" stroke="#fff" strokeWidth="1" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M13.7598 7.79006C13.7598 5.49006 12.8598 3.39006 11.9598 2.19006C11.6598 1.89006 11.1598 1.99006 11.0598 2.39006C10.6598 3.89006 9.45977 7.09006 6.55977 10.8901C2.85977 15.6901 6.25978 20.8901 9.75978 21.8901C11.6598 22.3901 9.25979 20.8901 8.95979 17.7901C8.65979 13.8901 13.7598 10.9901 13.7598 7.79006Z" stroke="#fff" strokeWidth="1" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round" />
              </g>
              <defs>
              ...
              {tabs.map((tab) => (
                <div
                  ...
                  className={`
                    ...
                    ${tab.isActive ? "bg-[#201C1C]" : "hover:bg-[#242424]"}
                  `}
